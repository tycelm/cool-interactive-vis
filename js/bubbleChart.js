...
class BubbleChart {
    ...
    setFilter(type) { this.filterType = type; }

    getFilteredData() {
        let vis = this;
        let filtered = vis._data.filter((d) => d.genres.includes("Action"));
    ...
    initVis() {
        ...
        const x = d3.scaleLinear()
            .domain([0, Math.min(d3.max(filtered, (d) => d.price), 72)])
    ...
    updateVis(timeDomain) {
        let vis = this;
        const filtered = vis.getFilteredData().filter(
            (d) => d.year >= timeDomain[0] && d.year <= timeDomain[1]
        );
